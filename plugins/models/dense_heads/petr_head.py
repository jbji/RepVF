...
class PETRHead(AnchorFreeHead):
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        self._save_qkv_hook(
            query_embeds, pos_embed, x, masks, reference_points, img_metas
        )
    ...
    def _save_qkv_hook(
        self, query_embeds, pos_embed, x, masks, reference_points, img_metas
    ):
        if os.environ.get("SAVE_QKV") == "True":
            npdump_nocheck(
                out_type="query_embeds",
                np_obj=query_embeds.cpu().numpy(),
                img_metas=img_metas,
            )
            npdump_nocheck(
                out_type="pos_embed",
            ...
            npdump_nocheck(
                out_type="reference_point",
                np_obj=reference_points.cpu().numpy(),
                img_metas=img_metas,
            )
