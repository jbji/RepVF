...
class ResizeCropFlipImage(object):
    ...
    def _sample_augmentation(self):
        ...
        if self.training:
            ...
            # bug fix begin: ensure resize is within resize_lim so that model always sees
            resize_lim = min(*self.data_aug_conf["resize_lim"])
            resize = max(resize, resize_lim)
            # bug fix end.
            resize_dims = (int(W * resize), int(H * resize))
            newW, newH = resize_dims
...
@PIPELINES.register_module()
class CustomResizeCropFlipImage(ResizeCropFlipImage):
    def __init__(
        self,
        data_aug_conf=None,
        training=True,
        ...
        super().__init__(data_aug_conf, training)
        self.fixed_resize = fixed_resize
        self.for_lane, self.for_bbox = not for_bbox, for_bbox
    ...
    def __call__(self, results):
        ...
        for i in range(N):
            ...
            new_imgs.append(np.array(img).astype(np.float32))
            if self.for_bbox:
                results["calib"]["intrinsics"][i][:3, :3] = (
                    ida_mat @ results["calib"]["intrinsics"][i][:3, :3]
                )
            if self.for_lane:
                results["calib"]["intrinsic"][:3, :3] = (
                    ida_mat @ results["calib"]["intrinsic"][:3, :3]
                )
        ...
        if self.for_lane:
            # instrinsic [3,3]; extrinsic [4,4]
            # lidar2img is actually ground space ot image space
            E = results["calib"]["extrinsic"]  # 4*4
            K = results["calib"]["intrinsic"]  # 3*3
            ...
            E_inv = np.linalg.inv(E)  # ground2cam
            P_g2im = np.matmul(K, E_inv)

            results["ground2img"] = [P_g2im]
            # [x] FIXME: lidar2img and ground2img need to be explored to determine if they are the same
            results["lidar2img"] = results["ground2img"]
            # stack a [0,0,0,1]

        if self.for_bbox:
            results["lidar2img"] = []
            for i in range(len(results["calib"]["extrinsics"])):
                E_inv = np.linalg.inv(results["calib"]["extrinsics"][i])
                K = results["calib"]["intrinsics"][i]
                K = np.hstack((K, np.zeros((3, 1))))
                K = np.vstack((K, np.array([0, 0, 0, 1])))
                results["lidar2img"].append(K @ E_inv)
    ...
    def _img_transform(self, img, resize, resize_dims, crop, flip, rotate):
        ...
        # Non-random resize (fixed resize)
        if self.fixed_resize is not None:
            img = img.resize(self.fixed_resize)
            fixed_resize_ratio = torch.Tensor(
                [self.fixed_resize[0] / img.size[0], self.fixed_resize[1] / img.size[1]]
            )
            ida_rot = torch.diag(fixed_resize_ratio)
...
class GlobalRotScaleTransImage(object):
    ...
    def rotate_bev_along_z(self, results, angle):
        ...
        for view in range(num_view):
            ...
            results["extrinsics"][view] = (
                rot_mat_inv.T @ torch.tensor(results["extrinsics"][view]).float()
            ).numpy()
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        for view in range(num_view):
            ...
            results["extrinsics"][view] = (
                torch.tensor(rot_mat_inv.T @ results["extrinsics"][view]).float()
            ).numpy()
        return


@PIPELINES.register_module()
class CustomGlobalRotScaleTransImage(GlobalRotScaleTransImage):
    def __call__(self, results):
        """Call function to pad images, masks, semantic segmentation maps.
        Args:
            results (dict): Result dict from loading pipeline.
        ...
        m1 = self.rotate_bev_along_z(results, rot_angle)
        if self.reverse_angle:
            rot_angle *= -1
        ...
        # random scale
        scale_ratio = np.random.uniform(*self.scale_ratio_range)
        m2 = self.scale_xyz(results, scale_ratio)
        if "gt_bboxes_3d" in results:
            results["gt_bboxes_3d"].scale(scale_ratio)

        # lane3d support
        if "gt_lanes_3d" in results:
            # Assume results['gt_lanes_3d']['lane_pts'][0] is a numpy array with shape (10, 3)
            for i in range(len(results["gt_lanes_3d"]["lane_pts"])):
                lane_pts = results["gt_lanes_3d"]["lane_pts"][i]

                # Homogeneous coordinates
                lane_pts_homo = np.hstack(
                    [lane_pts, np.ones((lane_pts.shape[0], 1))]
                )  # Shape will be (10, 4)

                # Apply Rotation
                rotated_lane_pts_homo = (
                    lane_pts_homo @ m1.T.numpy()
                )  # Assuming rot_mat is the rotation tensor in your code
                rotated_lane_pts = rotated_lane_pts_homo[:, :3]

                # Apply Scaling
                scaled_lane_pts_homo = (
                    lane_pts_homo @ m2.T.numpy()
                )  # Assuming rot_mat_inv is the scaling tensor in your code
                scaled_lane_pts = scaled_lane_pts_homo[:, :3]

                # Update results
                results["gt_lanes_3d"]["lane_pts"][i] = scaled_lane_pts
    ...
    def rotate_bev_along_z(self, results, angle):
        ...
        for view in range(num_view):
            ...
            results["calib"]["extrinsics"][view] = (
                rot_mat_inv.T
                @ torch.tensor(results["calib"]["extrinsics"][view]).float()
            ).numpy()
    ...
    def scale_xyz(self, results, scale_ratio):
        ...
        for view in range(num_view):
            ...
            results["calib"]["extrinsics"][view] = (
                torch.tensor(
                    rot_mat_inv.T @ results["calib"]["extrinsics"][view]
                ).float()
            ).numpy()
        return rot_mat
