...
class ConvertLaneToAnchors_FGAssump:
    '''
    ...
    def __init__(self,anchor_params_dict=None, prune_y_range=(0,200), y_ref=5, x_off_std=np.ones(10) , z_std=np.ones(10), is_calc_mode=False):
        
        # these parameters are used to generate anchors
        self.x_min, self.x_max, self.x_ratio = anchor_params_dict['x_range']
        self.y_min, self.y_max = anchor_params_dict['y_range']
        self.prune_y_min, self.prune_y_max = prune_y_range
        self.y_ref = y_ref
        self.anchor_y_steps = anchor_params_dict['anchor_y_steps']
        
        self.anchor_grid_x = anchor.AnchorGrid(**anchor_params_dict).anchor_grid_x
        
        # for gt anchor generation standardize
        self._x_off_std, self._z_std = np.array(x_off_std), np.array(z_std)
        
        # Initialize running statistics for dynamic calculations
        self.sample_count = np.zeros(len(self.anchor_y_steps))
        self.running_mean_x = np.zeros(len(self.anchor_y_steps))
        self.running_mean_z = np.zeros(len(self.anchor_y_steps))
        self.running_ssq_x = np.zeros(len(self.anchor_y_steps))
        self.running_ssq_z = np.zeros(len(self.anchor_y_steps))
    ...
    def __call__(self, results):
        ...
        # name alias
        gt_lanes, gt_visibility, gt_category = gt_lane_pts, gt_lane_visibility, gt_lane_category
        
        # camera parameters for anchor generation
        cam_K = results['calib']['intrinsic']
        cam_E = results['calib']['extrinsic']
        # g2im stands for ground to image
        P_g2im = self.projection_g2im_extrinsic(cam_E, cam_K) # ground space to camera plane
        H_g2im = self.homograpthy_g2im_extrinsic(cam_E, cam_K) # ground plane to camera plane
        ...
        for i in range(len(gt_lanes)):
            ...
            ass_id, x_off_values, z_values, visibility_vec = \
        ...
        if not self.is_calc_mode:
            ...
            if gt_anchors:
                ...
                lane_x_off = np.vstack([lane[:, 0] for lane in gt_anchors])
                ...
                self.update_statistics(lane_x_off, lane_z, visibility_weights)
    ...
    def __repr__(self):
        repr_str = (f'{self.__class__.__name__}('
                    ...
                    f'lane_x_off_std={self._x_off_std}, '
    ...
    def update_statistics_old(self, lane_x_off, lane_z, visibility_weights):
        ...
        for x, z, weight in zip(lane_x_off, lane_z, visibility_weights):
            for i, w in enumerate(weight):
                ...
                self.running_ssq_x[i] += w * (x[i] ** 2)
                self.running_ssq_z[i] += w * (z[i] ** 2)

        # Calculate standard deviations from running weighted sum of squares
        self._x_off_std = np.sqrt(self.running_ssq_x / self.sample_count - self.running_mean_x ** 2)
        self._z_std = np.sqrt(self.running_ssq_z / self.sample_count - self.running_mean_z ** 2)
    
    # weighted RMS
    def update_statistics(self, lane_x_off, lane_z, visibility_weights):
        # shape (N, 10), N is the count of lanes, 10 is the anchor number
        # expected output: (10) for _x_off_std & _z_std.
            for x, z, weight in zip(lane_x_off, lane_z, visibility_weights):
                for i, w in enumerate(weight):
            ...
            # Calculate standard deviations from running weighted sum of squares
            self._x_off_std = np.sqrt(self.running_ssq_x / self.sample_count)
    ...
    def convert_label_to_anchor(self, laneline_gt):
        ...
        valid_indices = np.logical_and(
            np.logical_and(gt_lane_3d[:, 1] > self.prune_y_min, 
                           gt_lane_3d[:, 1] < self.prune_y_max), 
            np.logical_and(
                gt_lane_3d[:, 0] > self.x_ratio * self.x_min,
                gt_lane_3d[:, 0] < self.x_ratio * self.x_max)
            )
        gt_lane_3d = gt_lane_3d[valid_indices, ...]
        # use more restricted range to determine deletion or not
        if gt_lane_3d.shape[0] < 2 or \
            np.sum(np.logical_and(gt_lane_3d[:, 0] > self.x_min,
        ...
        x_values, z_values, visibility_vec = \
            self.resample_laneline_in_y(gt_lane_3d, self.anchor_y_steps, out_vis=True)
        ...
        ass_id = np.argmin(
            np.linalg.norm(
                np.multiply(self.anchor_grid_x - x_values, visibility_vec), axis=1)
            )
...
@PIPELINES.register_module()
class GenerateAnchorGroundTruth:
    
    def __init__(self, anchor_params_dict, num_category_toggle):
        self.anchor_grid = anchor.AnchorGrid(**anchor_params_dict)
        self.anchor_num = self.anchor_grid.anchor_num
        self.num_types = 1
        self.num_y_steps = self.anchor_grid.num_y_steps
        self.num_category = 21 if num_category_toggle else 22 # merge road edge
        self.anchor_dim = 3 * self.num_y_steps + self.num_category
    ...
    def __call__(self, results):
        ...
        results['gt_anchor_lanes'].update({
            'packed': gt_anchor
        })
        return results
