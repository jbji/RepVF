...
class ConvertLaneToAnchors_FGAssump:
    ...
    def update_statistics_old(self, lane_x_off, lane_z, visibility_weights):
        ...
        self._x_off_std = np.sqrt(
            self.running_ssq_x / self.sample_count - self.running_mean_x**2
        )
        self._z_std = np.sqrt(
    ...
    def convert_label_to_anchor(self, laneline_gt):
        ...
        valid_indices = np.logical_and(
            np.logical_and(
                gt_lane_3d[:, 1] > self.prune_y_min, gt_lane_3d[:, 1] < self.prune_y_max
            ),
            np.logical_and(
        ...
        if (
            ...
            or np.sum(
                np.logical_and(
                    gt_lane_3d[:, 0] > self.x_min, gt_lane_3d[:, 0] < self.x_max
                )
            )
            < 2
        ):
        ...
        x_values, z_values, visibility_vec = self.resample_laneline_in_y(
            gt_lane_3d, self.anchor_y_steps, out_vis=True
        )
...
class GenerateAnchorGroundTruth:
    ...
    def __call__(self, results):
        ...
        for i in range(len(gt_lanes)):
            ...
            gt_anchor[ass_id, 0, 2 * self.num_y_steps : 3 * self.num_y_steps] = (
                visibility
            )
