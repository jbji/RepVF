...
class LaneEval(object):
    ...
    def bench(
        ...
        gt_lanes = [
            prune_3d_lane_by_visibility(np.array(gt_lane), np.array(gt_visibility[k]))
        ...
        pred_lanes = [
            prune_3d_lane_by_range(np.array(lane), self.x_min, self.x_max)
            for lane in pred_lanes
        ]
        ...
        for i in range(cnt_gt):
            for j in range(cnt_pred):
                ...
                if np.sum(both_visible_indices[:close_range_idx]) > 0:
                    ...
                    ) / np.sum(both_visible_indices[:close_range_idx])
    ...
    def _to_openlane_coord(self, whatever_lanes):
        """
            Coordinate System Change
        Args:
            whatever_lanes (_type_): _description_
        """
        R = np.array([[0, -1, 0], [1, 0, 0], [0, 0, 1]], dtype=float)
        for i in range(len(whatever_lanes)):
            lane = whatever_lanes[i]
            whatever_lanes[i] = np.matmul(R, lane.T).T
        return whatever_lanes

    def bench_one_submit_runtime(
        self, ground_truth, predictions, prob_th=0.5, vis=False
    ):
        ann_file = ground_truth
        json_pred = predictions

        print("Loading gt pkl ...")
        meta = io.pickle_load(ann_file)

        if len(meta.keys()) != len(json_pred):
        ...
        for i, pred in tqdm(
            enumerate(json_pred), desc="Running Evaluation for OpenLaneV1"
        ):
            if i % 1000 == 0 or i == len(json_pred) - 1:
                print("eval:{}/{}".format(i + 1, len(json_pred)))
            if "identifier" not in pred or "lane_lines" not in pred:
                raise Exception("identifier or lane_lines not in some predictions.")
            identifier = pred["identifier"]

            pred_lanelines = pred["lane_lines"]
            pred_lanes = [
                np.array(lane["xyz"]) for i, lane in enumerate(pred_lanelines)
            ]
            pred_category = [
                int(lane["category"]) for i, lane in enumerate(pred_lanelines)
            ]
            ...
            # evaluate lanelines
            cam_extrinsic = np.array(gt["extrinsics"][0])
            cam_extrinsic = np.vstack((cam_extrinsic, np.array([0, 0, 0, 1])))
            gt_cam_height = cam_extrinsic[2, 3]
            gt_cam_pitch = 0
            waymo_to_stdcam = np.array([[0, -1, 0], [0, 0, -1], [1, 0, 0]], dtype=float)
            cam_intrinsic = gt["intrinsics"][0]
            cam_intrinsic = np.array(cam_intrinsic) @ waymo_to_stdcam
            ...
            for j, gt_lane_packed in enumerate(gt_lanes_packed):
                ...
                lane = np.matmul(cam_extrinsic, lane)
                lane = lane[0:3, :].T

                gt_lanes.append(lane)
                gt_visibility.append(lane_visibility)

                if self.merge_road_edge:
                    if gt_lane_packed["category"] == 21:
                        gt_category.append(20)
                    else:
                        gt_category.append(gt_lane_packed["category"])

            P_g2im = projection_g2im_extrinsic(cam_extrinsic, cam_intrinsic)

            pred_lanes = self._to_openlane_coord(pred_lanes)
            gt_lanes = self._to_openlane_coord(gt_lanes)
        ...
        x_error_close_avg = np.average(
            laneline_x_error_close[laneline_x_error_close > -1 + 1e-6]
        )
        x_error_far_avg = np.average(
            laneline_x_error_far[laneline_x_error_far > -1 + 1e-6]
        )
        z_error_close_avg = np.average(
            laneline_z_error_close[laneline_z_error_close > -1 + 1e-6]
        )
        z_error_far_avg = np.average(
            laneline_z_error_far[laneline_z_error_far > -1 + 1e-6]
        )
    ...
    def bench_one_submit(self, pred_dir, gt_dir, test_txt, prob_th=0.5, vis=False):
        ...
        for i, pred in enumerate(json_pred):
            ...
            cam_extrinsics[:3, :3] = np.matmul(
                np.matmul(np.matmul(np.linalg.inv(R_vg), cam_extrinsics[:3, :3]), R_vg),
                R_gc,
            )
