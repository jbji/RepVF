# Taken from: https://github.com/OpenDriveLab/PersFormer_3DLane 
# ==============================================================================
...
try:
    ...
    # you can install nms from persformer https://github.com/OpenDriveLab/PersFormer_3DLane if you have need
    pass
...
def define_args():
    ...
    parser.add_argument(
        "--data_dir", type=str, help="The path of dataset json files (annotations)"
    )
    parser.add_argument(
        "--dataset_dir", type=str, help="The path of dataset image files (images)"
    )
    parser.add_argument(
        "--save_path", type=str, default="data_splits/", help="directory to save output"
    )
    parser.add_argument(
        "--use_memcache",
        type=str2bool,
        ...
        const=True,
        default=True,
        help="if use memcache",
    )
    # Dataset settings
    parser.add_argument(
        "--org_h", type=int, default=1080, help="height of the original image"
    )
    parser.add_argument(
        "--org_w", type=int, default=1920, help="width of the original image"
    )
    parser.add_argument("--crop_y", type=int, default=0, help="crop from image")
    parser.add_argument(
        "--cam_height", type=float, default=1.55, help="height of camera in meters"
    )
    parser.add_argument(
        "--pitch",
        type=float,
        default=3,
        help="pitch angle of camera to ground in centi degree",
    )
    parser.add_argument(
        "--fix_cam",
        type=str2bool,
        ...
        const=True,
        default=False,
        help="if to use fix camera",
    )
    parser.add_argument(
        "--no_3d",
        action="store_true",
        help="if a dataset include laneline 3D attributes",
    )
    parser.add_argument(
        "--no_centerline", action="store_true", help="if a dataset include centerline"
    )
    parser.add_argument(
        "--num_category",
        type=int,
        default=2,
        help="number of lane category, including background",
    )
    # PersFormer settings
    ...
    parser.add_argument(
        "--ipm_h", type=int, default=208, help="height of inverse projective map (IPM)"
    )
    parser.add_argument(
        "--ipm_w", type=int, default=128, help="width of inverse projective map (IPM)"
    )
    parser.add_argument(
        "--resize_h",
        type=int,
        default=360,
        help="height of the resized image (input of net)",
    )
    parser.add_argument(
        "--resize_w",
        type=int,
        default=480,
        help="width of the resized image (input of net)",
    )
    parser.add_argument(
        "--y_ref",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.5,
        help="probability threshold for selecting output lanes",
    )
    parser.add_argument(
        "--encoder",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="number of channels after encoder",
    )
    parser.add_argument(
        "--num_proj", type=int, default=4, help="number of projection layers"
    )
    parser.add_argument(
        "--num_att", type=int, default=3, help="number of attention encoding layers"
    )
    parser.add_argument(
        "--use_proj",
        type=str2bool,
    ...
    # LaneATT settings
    parser.add_argument(
        "--max_lanes", type=int, default=6, help="max lane number detection in LaneATT"
    )
    parser.add_argument(
        "--S", type=int, default=72, help="max sample number in img height"
    )
    parser.add_argument(
        "--anchor_feat_channels",
        type=int,
        default=64,
        help="number of anchor feature channels",
    )
    parser.add_argument(
        "--cls_loss_weight",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.1,
        help="confidence threshold for selecting output 2D lanes",
    )
    parser.add_argument(
        "--vis_th",
        type=float,
        default=0.1,
        help="visibility threshold for output 2D lanes",
    )
    parser.add_argument(
        "--loss_att_weight",
        type=float,
    ...
    parser.add_argument(
        "--nepochs", type=int, default=100, help="total numbers of epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--channels_in", type=int, default=3, help="num channels of input image"
    )
    parser.add_argument(
        "--test_mode", action="store_true", help="prevents loading latest saved model"
    )
    parser.add_argument(
        "--start_epoch", type=int, default=0, help="prevents loading latest saved model"
    )
    parser.add_argument(
        "--evaluate", action="store_true", help="only perform evaluation"
    )
    parser.add_argument(
        "--resume", type=str, default="", help="resume latest saved run"
    )
    parser.add_argument(
        "--vgg_mean",
        type=float,
    ...
    # Optimizer settings
    parser.add_argument(
        "--optimizer", type=str, default="adamw", help="adam/adamw/sgd/rmsprop"
    )
    parser.add_argument(
        "--weight_init",
        type=str,
    ...
    parser.add_argument(
        "--lr_decay", action="store_true", help="decay learning rate with rule"
    )
    parser.add_argument(
        "--niter", type=int, default=900, help="# of iter at starting learning rate"
    )
    parser.add_argument(
        "--niter_decay",
        type=int,
    ...
    parser.add_argument(
        "--lr_policy",
        default=None,
        help="learning rate policy: lambda|step|cosine|cosine_warm",
    )
    parser.add_argument(
        "--gamma",
        type=float,
    ...
    parser.add_argument(
        "--T_max", type=int, default=10, help="maximum number of iterations"
    )
    parser.add_argument(
        "--T_0",
        type=int,
    ...
    parser.add_argument(
        "--T_mult", type=int, default=1, help="a factor increases T_i after a restart"
    )
    parser.add_argument(
        "--eta_min", type=float, default=1e-3, help="minimum learning rate"
    )
    parser.add_argument(
        "--clip_grad_norm", type=int, default=0, help="performs gradient clipping"
    )
    # CUDNN usage
    parser.add_argument(
        "--cudnn",
...
class Visualizer:
    def __init__(self, args):
        ...
        if self.use_default_anchor:
            self.anchor_x_steps = np.linspace(
                x_min, x_max, np.int(args.ipm_w / 8), endpoint=True
            )
        else:
    ...
    def draw_on_img_category(
        self, img, pred_anchors, gt_anchors, P_g2im, draw_type="laneline"
    ):
        """
        ...
        for j in range(lane_anchor.shape[0]):
            ...
            line_pred = {}
            lane_cate = np.argmax(
                lane_anchor[j, self.anchor_dim - self.num_category : self.anchor_dim]
            )
            if draw_type == "laneline" and lane_cate != 0:
                x_offsets = lane_anchor[j, : self.num_y_steps]
                x_3d = x_offsets + self.anchor_x_steps[j]
                if P_g2im.shape[1] == 3:
                    x_2d, y_2d = homographic_transformation(
                        P_g2im, x_3d, self.anchor_y_steps
                    )
                    if not self.use_default_anchor:
                        anchor_x_2d, _ = homographic_transformation(
                            P_g2im, self.anchor_x_steps[j], self.anchor_y_steps
                        )
                else:
                    z_3d = lane_anchor[j, self.num_y_steps : 2 * self.num_y_steps]
                    x_2d, y_2d = projective_transformation(
                        P_g2im, x_3d, self.anchor_y_steps, z_3d
                    )
                ...
                if lane_cate == 1:  # white dash
                    ...
                    plt.plot(
                        x_2d, y_2d, "white", linestyle=(0, (10, 10)), lw=2, alpha=0.5
                    )
                    ...
                    plt.plot(
                        x_2d, y_2d, "white", linestyle=(0, (20, 10)), lw=2, alpha=0.5
                    )
    ...
    def draw_on_ipm_category(
        self, im_ipm, pred_anchors, gt_anchors, draw_type="laneline"
    ):
        ...
        for j in range(lane_anchor.shape[0]):
            ...
            if draw_type == "laneline" and lane_cate != 0:
                ...
                if self.no_3d:
                    ...
                    visibility = lane_anchor[
                        j, 2 * self.num_y_steps : 3 * self.num_y_steps
                    ]

                # compute lanelines in ipm view
                x_ipm, y_ipm = homographic_transformation(
                    self.H_g2ipm, x_g, self.anchor_y_steps
                )
                if not self.use_default_anchor:
                    anchor_x_ipm, _ = homographic_transformation(
                        self.H_g2ipm, self.anchor_x_steps[j], self.anchor_y_steps
                    )
                ...
                if lane_cate == 1:  # white dash
                    ...
                    plt.plot(
                        x_2d, y_2d, "white", linestyle=(0, (10, 10)), lw=10, alpha=0.5
                    )
                elif lane_cate == 2:  # white solid
                    ...
                    plt.plot(
                        x_2d, y_2d, "white", linestyle=(0, (20, 10)), lw=10, alpha=0.5
                    )
                elif lane_cate == 8:  # yellow-solid
    ...
    def draw_3d_curves_category(
        self, pred_anchors, gt_anchors, h_cam, draw_type="laneline"
    ):
        ...
        for j in range(lane_anchor.shape[0]):
            ...
            if draw_type == "laneline" and lane_cate != 0:
                ...
                if len(x_gflat) > 0:
                    ...
                    x_g, y_g = transform_lane_gflat2g(
                        h_cam,
                        x_gflat,
                    ...
                    if lane_cate == 1:  # white dash
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "mediumpurple", lw=3, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            x_g,
                            y_g,
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "mediumorchid", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15,
                            y_g,
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "lightskyblue", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15, y_g, z_g, "white", lw=0.5, alpha=1
                        )
                        ax.plot(
                            np.array(x_g) - 0.15, y_g, z_g, "white", lw=0.5, alpha=1
                        )
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "hotpink", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15,
                            y_g,
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "salmon", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15,
                            y_g,
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "lightcoral", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15, y_g, z_g, "white", lw=0.5, alpha=1
                        )
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "coral", lw=4, alpha=0.8, label="pred"
                        )
                        ax.plot(
                            np.array(x_g) + 0.15,
                            y_g,
                        ...
                        (line1,) = ax.plot(
                            x_g, y_g, z_g, "forestgreen", lw=3, alpha=0.8
                        )
                        ax.plot(x_g, y_g, z_g, "white", lw=1, alpha=0.5)
                    elif lane_cate == 20 or lane_cate == 21:  # road
        ...
        for j in range(lane_anchor.shape[0]):
            ...
            if (
                ...
                and np.argmax(
                    lane_anchor[
                        j, self.anchor_dim - self.num_category : self.anchor_dim
                    ]
                )
                != 0
            ):
                ...
                if len(x_gflat) > 0:
                    ...
                    if z_min == -1 and z_max == 1:
                        ...
                        if min(z_g) < z_min:
                            z_min = min(z_g)
                    (line2,) = ax.plot(
                        x_g, y_g, z_g, lw=2, c="yellowgreen", alpha=1, label="gt"
                    )
        ...
        ax.zaxis.set_major_locator(
            plt.MultipleLocator(max(0.1, round((z_max - z_min) * 2 / 5, 1)))
        )
        ax.view_init(20, -60)
        ...
        for i in range(idx.shape[0]):
            ...
            if self.use_default_anchor:
                for j in range(1, self.num_category + 1):
                    pred_anchors[:, self.anchor_dim - j] = nms_1d(
                        pred_anchors[:, self.anchor_dim - j]
                    )
                    if not self.no_centerline:
                        pred_anchors[:, 2 * self.anchor_dim - j] = nms_1d(
                            pred_anchors[:, 2 * self.anchor_dim - j]
                        )
                        pred_anchors[:, 3 * self.anchor_dim - j] = nms_1d(
                            pred_anchors[:, 3 * self.anchor_dim - j]
                        )
            ...
            if "openlane" in self.dataset_name:
                ...
                H_g2im_pred = homograpthy_g2im(
                    pred_cam_pitch[i], pred_cam_height[i], intrinsics[i]
                )
            ...
            im_laneline = im.copy()
            fig = self.draw_on_img_category(
                im_laneline, pred_anchors, gt_anchors, P_gt, "laneline"
            )
            ipm_laneline = im_ipm.copy()
            fig2 = self.draw_on_ipm_category(
                ipm_laneline, pred_anchors, gt_anchors, "laneline"
            )
            fig3 = self.draw_3d_curves_category(
                pred_anchors, gt_anchors, extrinsics[i][2, 3], "laneline"
            )
            ...
            if evaluate:
                ...
                filename = self.save_path + "/example/{}/epoch-{}/segment-{}_{}".format(
                    train_or_val,
                    epoch,
...
def resample_laneline_in_x_with_vis(input_lane, x_steps, vis_vec):
    """
        Interpolate x, z values at each anchor grid, including those beyond the range of input lnae y range
    :param input_lane: N x 2 or N x 3 ndarray, one row for a point (x, y, z-optional).
                       It requires y values of input lane in ascending order
    ...
    f_vis = interp1d(input_lane[:, 0], vis_vec, fill_value="extrapolate")

    y_values = f_y(x_steps)
    z_values = f_z(x_steps)
    vis_values = f_vis(x_steps)

    x_values = x_steps[vis_values > 0.5]
    y_values = y_values[vis_values > 0.5]
    z_values = z_values[vis_values > 0.5]
    return np.array([x_values, y_values, z_values]).T
...
def homography_im2ipm_norm(
    ...
    x_2d, y_2d = homographic_transformation(
        H_g2im, top_view_region[:, 0], top_view_region[:, 1]
    )
...
def nms_bev(batch_output_net, args):
    ...
    anchor_x_steps = (
        ...
        if not args.use_default_anchor
            ...
            args.top_view_region[0, 0],
            args.top_view_region[1, 0],
            np.int(args.ipm_w / 8),
        )
    )

    batch_output_net = batch_output_net.reshape(
        batch_output_net.shape[0], batch_output_net.shape[1], anchor_dim
    )
    # print("cate before softmax: ", batch_output_net[:, :, anchor_dim-args.num_category:].shape)
    # print(batch_output_net[0, :16, :])
    batch_output_net[:, :, anchor_dim - args.num_category :] = softmax(
        batch_output_net[:, :, anchor_dim - args.num_category :], axis=2
    )
    ...
    for i, output_net in enumerate(batch_output_net):
        ...
        for j, output_anchor in enumerate(output_net):
            ...
            visible_yid = np.where(
                output_anchor[2 * args.num_y_steps : 3 * args.num_y_steps]
                > args.prob_th
            )[0]
            # print("vis points: ", len(visible_yid))
            if (
                np.argmax(output_anchor[anchor_dim - args.num_category :])
                == anchor_dim - args.num_category
                or len(visible_yid) < 2
            ):
                # print("skip")
                continue
            pre_nms_valid_anchor_id.append(j)

            scores[valid_count] = np.max(
                output_anchor[anchor_dim - args.num_category + 1 :]
            ).item()
            ...
            output_net_nms[valid_count, 5 : 5 + args.num_y_steps] = torch.from_numpy(
                output_anchor[: args.num_y_steps] + anchor_x_steps[j]
            )
        ...
        keep, num_to_keep, _ = nms(
            output_net_nms, scores, overlap=args.nms_thres_3d, top_k=args.max_lanes
        )
...
def unit_update_projection_extrinsic(args, extrinsics, intrinsics):
    ...
    for i in range(batch_size):
        _M, _M_inv = homography_im2ipm_norm(
            args.top_view_region,
            np.array([args.org_h, args.org_w]),
            args.crop_y,
            np.array([args.resize_h, args.resize_w]),
            None,
            None,
            intrinsics[i].data.cpu().numpy(),
            extrinsics[i].data.cpu().numpy(),
        )
...
def unit_update_projection(
    args, cam_height, cam_pitch, intrinsics=None, extrinsics=None
):
    """
...
def unit_update_projection_for_data_aug(
    args, aug_mats, _M_inv, _S_im_inv=None, _S_im=None
):
    """
    ...
    if _S_im_inv is None:
        _S_im_inv = torch.from_numpy(
            np.array(
                [
                    [1 / np.float(args.resize_w), 0, 0],
    ...
    if _S_im is None:
        _S_im = torch.from_numpy(
            np.array(
                [[args.resize_w, 0, 0], [0, args.resize_h, 0], [0, 0, 1]],
                dtype=np.float32,
            )
        ).cuda()
